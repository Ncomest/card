...
import ButtonDark2 from "../../components/button/button_dark_2";
...
export const CreateDeckPage = () => {
    ...
    const fetchCards = async () => {
      try {
        const data = await fetchApi({
          API_URI: `/api/create-new-deck/v1/all-cards?page=${page}&limit=5`,
        });
        setCardsData(data);
      } catch (error) {
  ...
  const rightCardListMap = newDeck.map((el, i) => (
    <CardListInCreateDeck key={i} deckList={el} index={i} />
  ));
  //>===========RightSide_end============//
  ...
  return (
    <ComponentStyle>
      <LeftSideStyle>
        ...
        <PagesContainerStyle>
          {page >= 2 && <ButtonDarkStone text={"назад"} onClick={decrement} />}
          <p>{page}{" из "}{totalPages}</p>
          {page < totalPages && (<ButtonDarkStone text={"вперед"} onClick={increment} />)}
        </PagesContainerStyle>
      </LeftSideStyle>
