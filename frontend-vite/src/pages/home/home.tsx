...
// import { getDataTable, getDataTableLongPolling } from "@/api/api";
import type { ICard, ICardTable, IDrag } from "@/types/types";
...
const Home: React.FC = () => {
  ...
  const longPull = async () => {
    ...
    try {
      const response = await fetchApi({ API_URI: "/api/table/update" });
  ...
  useEffect(() => {
    const fetchTable = async () => {
      try {
        const response = await fetchApi({ API_URI: "/api/table" });
        setTable(response);
        console.log("data стола от useEffect (home.tsx)", response);
      } catch (error) {
